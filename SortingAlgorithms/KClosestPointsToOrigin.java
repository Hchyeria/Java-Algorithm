...
public class KClosestPointsToOrigin {
    ...
    private void quickSelect(int[][] points, int left, int right, int k) {
        // need to check the valid boundary
        if (left >= right) {
            return;
        }
        ...
        if (index < k) {
            // no need to change the value of k
    ...
    private int partition(int[][] points, int left, int right) {
        ...
        while (i <= j) {
            // should use points[right] to compare
    ...
    public static void main(String[] args) {
        ...
        int[][] a = {{1,3}, {-2,2}};
        System.out.println(Arrays.deepToString(kClosestPointsToOrigin.kClosest2(a, 1)));
    }
}
