...
public class AllFactorsOfANumber {
    ...
    // suppose target have k factors, and there are at most k layers,
    // and each node has at most log(target) branch, so O(log(target) * k)
    // Space Complexity: O(k)

    public static void main(String[] args) {
        AllFactorsOfANumber allFactorsOfANumber = new AllFactorsOfANumber();
        System.out.println(allFactorsOfANumber.factors(12));
    }
}
