...
//                        continue;
//                    }
//                    next.distance[j] = Math.min(next.distance[j], next.distance[j - 1] + 1);
//                    next.distance[j] = Math.min(next.distance[j], cur.distance[j - 1] + 1);
//                    if (s.charAt(i - 1) == target.charAt(j - 1)) {
//                        next.distance[j] = Math.min(next.distance[j], cur.distance[j - 1]);
//                    }
                }
...
//            }
    ...
    public static void main(String[] args) {
        KEditDistanceDFS kEditDistance = new KEditDistanceDFS();
        String[] s = {"as","ab","cf","da","ee","e","adee","eeda"};
        String target = "eefab";
        int k = 3;
        System.out.println(kEditDistance.kDistance(s, target, k));
    }
}

class KEditDistanceDFS {
    private static class TrieNode {
        TrieNode[] children;
        String hasWord;
