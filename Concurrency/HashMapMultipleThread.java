...
public class HashMapMultipleThread<K, V> {
    // entry class
    // why public static??
    // 1. define a class Node for each Node in the HashMap
    // since it is closely bounded with HashMapMultipleThread<K, V> and we
    // may need access it from outer class (public), so we need to use
    // inner static class (Map.Entry<K, V>)
    // HashMapMultipleThread.Node to avoid rename collision
    ...
    public static class Node<K, V> {
        ...
        // node is in HashMap
        // HashMap has locked node
        // so these method not need to lock anymore
        // no public synchronized V getValue()
        // no public void setValue(V value)
        public V getValue() {
            return value;
        }
    ...
    public HashMapMultipleThread() {
        // call public HashMapMultipleThread(int capacity, float loadFactor)
        this(DEFAULT_CAPACITY, DEFAULT_LOAD_FACTOR);
    }

    public HashMapMultipleThread(int capacity) {
        // call public HashMapMultipleThread(int capacity, float loadFactor)
        this(capacity, DEFAULT_LOAD_FACTOR);
    }

    public HashMapMultipleThread(int capacity, float loadFactor) {
        if (capacity < 0) {
    ...
    // put/remove write size, size()/isEmpty() read size
    // need lock
    ...
    private void rehash() {
        HashMapMultipleThread<K, V> temp = new HashMapMultipleThread<>(array.length * 2, loadFactor);
        for (Node<K, V> node : array) {
            while (node != null) {
